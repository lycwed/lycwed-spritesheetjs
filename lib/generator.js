...
exports.generateImage = function (files, options, callback) {
	...
	exec(command.join(' '), function (err) {
		...
		exec('magick identify ' + filePath, function(err, stdout) {
			var message = err || stdout;
			if (err) {
				spinner.fail("Spritesheet identification fails...");
			} else {
				params = message.split(' ');
				message = ['format: ' + params[1], 'size: ' + params[2], 'weight: ' + params[2]].join()
				spinner.succeed("YEAH! Spritesheet successfully generated! " + message);
			}
				...
				tinify
				  .fromFile(filePath)
					.toFile(filePath).then(function() {
						spinner.succeed("WOOOOW! Spritesheet successfully tinified!");
						callback(null);
					}, function(err) {
						message = err;
