...
exports.trimImages = function (files, options, callback) {
	...
	async.eachSeries(files, function (file, next) {
		...
		var command = 'convert' + fuzz + ' -define png:exclude-chunks=date "' + file.originalPath + '" -bordercolor transparent -border 1 -trim "' + file.path + '"';
		exec(command, next);
	}, callback);
};
...
exports.generateImage = function (files, options, callback) {
	...
	var command = ['convert -define png:exclude-chunks=date -quality 0% ' + scale + ' -size ' + options.width + 'x' + options.height + ' xc:none'];
	...
	exec(command.join(' '), function (err) {
		...
		function getImageInfos(filePath, onFinish) {
			exec('magick identify ' + filePath, function(err, stdout) {
				if (err) {
					infos = 'unable to retrieve infos...';
				} else {
					params = stdout.split(' ');
					infos = ['format: ' + params[1], 'size: ' + params[2], 'weight: ' + params[6]].join(' / ');
				}
				onFinish(infos);
			});
		}

		getImageInfos(filePath, function(infos) {
