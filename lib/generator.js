...
exports.generateImage = function (files, options, callback) {
	...
	exec(command.join(' '), function (err) {
		...
		if (options.tinify) {
			console.log(cliSpinners.lycwed);
			...
			tinify
				...
				.toFile(filePathOptimized).then(function() {
					...
					if (err instanceof tinify.AccountError) {
						...
						callback('tinify: generated image seems to be the problem');
					} else if (err instanceof tinify.ServerError) {
						// Temporary issue with the Tinify API.
						callback('tinify: issue with the API');
					} else if (err instanceof tinify.ConnectionError) {
						// A network connection error occurred.
						callback('tinify: network connection error');
					} else {
